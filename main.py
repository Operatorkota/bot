...
# Definicja ról do stworzenia: (nazwa_roli, kolor)
ROLES_TO_CREATE = [
    ("Administracja", discord.Color.red()),
    ("Dyrektor Placówki", discord.Color.gold()),
    ("Special Response Unit(SRU)", discord.Color.dark_red()),
    ("Ochrona (SSC)", discord.Color.dark_blue()),
    ("Kontraktor", discord.Color.dark_green()),
    ("Obsługa (MT)", discord.Color.orange()),
    ("Orderly", discord.Color(0xFFFFFF)), # Biały
    ("Orderly Support", discord.Color.dark_grey()),
    ("Pacjent", discord.Color.light_grey()),
    ("paramedyk", discord.Color.blue()),
...
class MyClient(discord.Client):
    ...
    async def change_status(self):
        ...
        if rp_is_on:
            ...
            rp_role = discord.utils.get(guild.roles, name="Pacjent")
            rp_count = 0
            if rp_role:
...
async def setup_server(interaction: discord.Interaction):
    ...
    created_roles = {}
    for role_name, color in reversed(ROLES_TO_CREATE):
...
# --- Komenda /przenies ---
SECTOR_CHOICES = [
    app_commands.Choice(name=sector, value=sector) for sector in config.SECTOR_ROLE_IDS.keys()
] if hasattr(config, 'SECTOR_ROLE_IDS') else []
...
async def przenies(interaction: discord.Interaction, uzytkownik: discord.Member, do_sektora: app_commands.Choice[str], na_czas: str, z_powrotem_do: app_commands.Choice[str], powod: str):
    ...
    try:
        ...
        sector_roles_ids = config.SECTOR_ROLE_IDS
        temp_role_id = sector_roles_ids[do_sektora.value]
        return_role_id = sector_roles_ids[z_powrotem_do.value]
    except (AttributeError, KeyError):
        await interaction.followup.send("❌ Błąd konfiguracji. Upewnij się, że `PRZYPIS_CHANNEL_ID` i `SECTOR_ROLE_IDS` są poprawnie ustawione w `config.py`.", ephemeral=True)
        return
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    ...
    if payload.message_id == verification_message_id and str(payload.emoji) == "✅":
        ...
        role = discord.utils.get(guild.roles, name="Pacjent")
